...
import {
  ...
  ChevronLeft, ChevronRight, Settings, Check,
} from 'lucide-react'

const documentTypes = [
  { id: 'DIPLOME', label: 'Diplôme de fin d\'études', description: 'Certificat final de votre diplôme', icon: GraduationCap },
...
function DocTypeCard({ id, label, description, icon: Icon, selected, onSelect }: {
  id: string; label: string; description: string
  icon: React.ElementType; selected: boolean; onSelect: () => void
}) {
  return (
    <motion.button
      type="button"
      onClick={onSelect}
      whileHover={{ y: -2, boxShadow: '0 6px 24px rgba(181,103,26,0.12)' }}
      whileTap={{ scale: 0.99 }}
      transition={{ type: 'spring', stiffness: 400, damping: 22 }}
      className="relative flex min-h-[170px] flex-col gap-4 rounded-sm border-2 bg-white p-4 text-left sm:p-5"
      ...
      <div className="flex items-start justify-between">
        ...
        <motion.div
          animate={{
            ...
            borderColor: selected ? 'var(--color-orange)' : 'rgba(3,23,61,0.22)',
          }}
...
export default function NouvelleDemandeStep1Page() {
      ...
      >
        ...
        <div className="mb-6 text-center sm:mb-7">
          ...
          <p className="mt-1.5 text-[13px] sm:text-[14px]" style={{ color: 'rgba(3,23,61,0.52)' }}>
            Veuillez sélectionner le type de document académique que vous souhaitez obtenir.
          </p>
        </div>
        ...
        <div className="mt-5 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <motion.button
            type="button"
            onClick={() => selected && router.push(`/demande/nouvelle/resume?type=${selected}`)}
            disabled={!selected}
            ...
            style={{
              backgroundColor: selected ? 'var(--color-orange)' : 'rgba(181,103,26,0.18)',
